...
const productSchema = z.object({ categoryId: z.string().min(1), name: z.string().min(2).max(100), description: z.string().max(500).optional(), imageUrl: z.string().url().optional().or(z.literal('')), price: z.coerce.number().min(0).max(999999), preparationMin: z.coerce.number().int().min(0).max(999).optional(), badges: z.array(z.string().max(30)).max(6).optional(), allergens: z.array(z.string().max(40)).max(12).optional() });
...
export async function POST(request: Request) {
  ...
  const product = await db.product.create({ data: { tenantId: session.tenantId!, categoryId: parsed.data.categoryId, name: parsed.data.name, description: parsed.data.description, imageUrl: parsed.data.imageUrl || null, price: parsed.data.price, preparationMin: parsed.data.preparationMin, badges: parsed.data.badges || [], allergens: parsed.data.allergens || [], sortOrder: await db.product.count({ where: { tenantId: session.tenantId!, categoryId: parsed.data.categoryId } }) + 1 } });
