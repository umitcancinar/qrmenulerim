...
export default function PanelClient({
  ...
  const nav = (key: Tab, label: string) => (
    <button type="button" className={tab === key ? 'selected' : ''} onClick={() => { setError(''); setTab(key); }}>{label}</button>
  );
  ...
  return <main className="panel-page">
    <aside className="panel-sidebar">
      ...
      <nav>{nav('menu', '▦ Menü içerikleri')}{nav('appearance', '◉ Görünüm ve tema')}{nav('integration', '⌘ API bağlantısı')}{nav('analytics', '⌁ İçerik özeti')}{nav('account', '⚙ Hesap ayarları')}</nav>
    ...
    <section className="panel-main">
      ...
      {tab === 'integration' && <form ref={integrationForm} className="tenant-section" onSubmit={saveTenant}><span className="eyebrow">HARİCİ MENÜ KAYNAĞI</span><h2>Menü API bağlantısı</h2><p>Mevcut sisteminizdeki menü verisini güvenli biçimde bağlayın. Kaydetmeden önce kategori ve ürünlerin okunabildiğini test edebilirsiniz.</p><div className="form-grid"><label className="wide">Menü API adresi<input name="menuApiUrl" type="url" defaultValue={String(settings.menuApiUrl || '')} placeholder="https://api.ornek.com/menu/{{slug}}" /><small>Tam API adresini yazın. {'{{slug}}'} alanı bu işletmenin menü koduyla otomatik değiştirilir.</small></label><input type="hidden" name="menuApiEnabled" value="off" /><label className="check-label wide"><input name="menuApiEnabled" type="checkbox" defaultChecked={settings.menuApiEnabled === true} /> QR menü içeriğini bu API adresinden canlı olarak al</label>{integrationResult && <div className={`integration-result wide ${integrationResult.ok ? 'is-success' : 'is-error'}`} role="status"><strong>{integrationResult.ok ? 'Bağlantı doğrulandı' : 'Bağlantı doğrulanamadı'}</strong><span>{integrationResult.message}</span></div>}<div className="integration-note wide"><strong>Bağlantı açıkken</strong><span>Kategori, ürün, fiyat, görsel, hazırlık süresi, rozet ve porsiyon bilgileri API kaynağından okunur. Kaynak yanıt vermezse kayıtlı menünüz kesintisiz gösterilir.</span></div><div className="integration-note wide"><strong>Desteklenen veri yapısı</strong><span>JSON yanıtında categories dizisi bulunmalıdır. Ürünler kategori içindeki items veya products dizisinde ya da üst seviyedeki products dizisinde categoryId ile gönderilebilir.</span></div><div className="integration-note wide"><strong>Güvenlik ve sınırlar</strong><span>Yerel ağ adresleri engellenir, bağlantı sekiz saniyede zaman aşımına uğrar ve en fazla 2 MB büyüklüğündeki JSON yanıtları işlenir.</span></div></div>{error && <div className="form-error">{error}</div>}<footer className="integration-actions"><button type="button" className="secondary-action" disabled={testingIntegration || saving} onClick={testIntegration}>{testingIntegration ? 'Bağlantı test ediliyor…' : 'Bağlantıyı test et'}</button><button className="primary-action" disabled={saving || testingIntegration}>{saving ? 'Kaydediliyor…' : 'API bağlantısını kaydet'}</button></footer></form>}

      {tab === 'analytics' && <><section className="sa-stats"><div><span>Toplam kategori</span><strong>{categories.length}</strong><small>Menüde listelenen gruplar</small></div><div><span>Toplam ürün</span><strong>{productTotal}</strong><small>QR menüdeki içerikler</small></div><div><span>Öne çıkan</span><strong>{allProducts.filter((product) => product.isFeatured).length}</strong><small>Şefin seçkisi alanında</small></div></section><section className="menu-health"><div className="health-score"><strong>{healthScore}<small>/100</small></strong><span>Menü hazırlık puanı</span></div><div><span className="eyebrow">YAYINA HAZIRLIK</span><h2>Menünüzü güçlendirecek adımlar</h2><ul><li className={tenant.coverUrl ? 'done' : ''}>Kapak görseli</li><li className={tenant.description ? 'done' : ''}>İşletme açıklaması</li><li className={tenant.phone && tenant.address ? 'done' : ''}>İletişim bilgileri</li><li className={productTotal >= 3 ? 'done' : ''}>En az 3 ürün</li></ul></div></section></>}
